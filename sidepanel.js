...
function exportPDF() {
  ...
  var sectionsHtml = '';
  for (var i = 0; i < summary.sections.length; i++) {
    sectionsHtml += '<div class="section"><h2>' + escapeHtml(summary.sections[i].header) + '</h2><p>' + escapeHtml(summary.sections[i].body) + '</p></div>';
  }

  var html = '<!DOCTYPE html><html><head><meta charset="utf-8"><title>' + escapeHtml(data.title) + ' - Summary</title><style>' +
    'body { font-family: Georgia, serif; max-width: 640px; margin: 0 auto; padding: 40px 20px; color: #1a1a1a; }' +
    'h1 { font-size: 22px; margin-bottom: 4px; line-height: 1.3; }' +
    '.meta { font-size: 12px; color: #888; margin-bottom: 8px; }' +
    '.type-badge { display: inline-block; font-size: 10px; padding: 2px 10px; border-radius: 99px; background: #f0f0f0; color: #666; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 20px; }' +
    'hr { border: none; border-top: 1px solid #e0e0e0; margin: 20px 0; }' +
    '.section { margin-bottom: 20px; }' +
    'h2 { font-size: 13px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #7c5cfc; margin-bottom: 6px; }' +
    'p { font-size: 14px; line-height: 1.8; color: #333; margin: 0; }' +
    '.footer { margin-top: 32px; font-size: 11px; color: #bbb; text-align: center; }' +
    '@media print { body { padding: 20px; } }' +
    '</style></head><body>' +
    '<h1>' + escapeHtml(data.title) + '</h1>' +
    '<div class="meta">' + escapeHtml(data.url) + '</div>' +
    '<div class="meta">' + new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) + '</div>' +
    '<span class="type-badge">' + escapeHtml(summary.content_type) + '</span>' +
    '<hr>' +
    sectionsHtml +
    '<div class="footer">Generated by Universal Summarizer</div>' +
    '</body></html>';
  ...
  chrome.tabs.create({ url: url }, function(tab) {
    // The user can now Ctrl+P or use browser menu to Save as PDF
    // We'll inject a script to auto-trigger print after a short delay
    setTimeout(function() {
